...
def translate(req: TranslateRequest):
    ...
    try:
        # First translation
        system_prompt_first = f"You are a professional translator. Translate the following numbered list of texts that appear on a website from English to Traditional Chinese (zh_HK) and Simplified Chinese (zh_CN). For Traditional Chinese (zh_HK), use expressions and vocabulary as spoken and written by Cantonese speakers in Hong Kong. For Simplified Chinese (zh_CN), use expressions and vocabulary as spoken and written by Mainland China speakers. Please use written language, not spoken language. Do NOT translate word-for-word, adapt the meaning for clarity and naturalness in a web context. For each numbered text, provide translations for all languages in this format:\n\n1. [Original text]\n{format_example}\n\n2. [Next text]\n{format_example}\n\nReturn ONLY the translations in this exact format without any explanations.{hardcoded_examples}"
        ...
        if retranslate:
            ...
            system_prompt_re = f"You are a professional translator. The previous translations for these website texts were not satisfactory. For each numbered text, you are given the original English and the first translation for each target language. Provide a different, better translation for each language, using written language as appropriate for the region (zh_HK for Hong Kong Cantonese, zh_CN for Mainland China). Do NOT translate word-for-word; adapt the meaning for clarity and naturalness in a web context. For each numbered text, provide translations for all languages in this format:\n\n1. [Original text]\n   zh_HK: [improved translation]\n   zh_CN: [improved translation]\n\nReturn ONLY the improved translations in this exact format without any explanations.{hardcoded_examples}"
